 ...
 * @Author: Roohom
 * @Function: 自定义开发实现WordCount hash分区法，三列显示，字符 该字符的长度 该字符出现的次数
 * @Date: 2020/8/22 15:38
 * @Software: IntelliJ IDEA
 */
public class WordCountPartitionBean extends Configured implements Tool {

    public int run(String[] args) throws Exception {
        Job job = Job.getInstance(this.getConf(), "userCount");
        job.setJarByClass(WordCountPartitionBean.class);
...
//        conf.set("fs.defultFS","hdfs://192.168.88.221:8020");
        int status = ToolRunner.run(conf, new WordCountPartitionBean(), args);
        System.exit(status);
    }
    ...
    public static class WcReduce extends Reducer<Text, IntWritable, UserBean, IntWritable> {
        ...
        protected void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            ...
            this.outputKey.setAll(key.toString(),key.toString().length());
            //将累加的结果作为输出的value
