 ...
 * @Author: Roohom
 * @Function: 自定义开发实现WordCount, 使用官方hash分区法
 * @Date: 2020/8/22 15:38
 * @Software: IntelliJ IDEA
 */
public class MRWordCountPartition extends Configured implements Tool {

    public int run(String[] args) throws Exception {
        Job job = Job.getInstance(this.getConf(), "userCount");
        job.setJarByClass(MRWordCountPartition.class);
...
//        conf.set("fs.defultFS","hdfs://192.168.88.221:8020");
        int status = ToolRunner.run(conf, new MRWordCountPartition(), args);
        System.exit(status);
    }
